...
def test_deposit_to_casper(casper_chain, casper, pool, induct_validators_and_depositors,
                           depositor_privkeys, depositor_deposit_amount, operator, validation_addr):
    ...
    valcode_addr = validation_addr(operator["key"])

    casper_chain.mine(pool.DEPOSIT_END()-casper_chain.chain.head.number + 1)

    assert casper_chain.chain.head.number > pool.DEPOSIT_END()
    assert casper_chain.chain.state.get_balance(pool.address) > casper.MIN_DEPOSIT_SIZE()

    pool.deposit_to_casper(valcode_addr, sender=operator["key"])
