...
from utils.valcodes import generate_validation_code
...
@pytest.fixture
def casper_code():
    with open('./casper/casper/contracts/simple_casper.v.py') as f:
        return f.read()
...
def mk_validation_code():
    def mk_validation_code(address):
        return generate_validation_code(address)
...
def mk_vote():
    def mk_vote(validator_index, target_hash, target_epoch, source_epoch, privkey):
        ...
        v, r, s = utils.ecdsa_raw_sign(msghash, privkey)
...
def logout_from_casper(deposit_to_casper, mine_until, pool, mk_logout_msg_unsigned, casper, operator):
    def logout_from_casper():
        ...
        logout_msg = mk_logout_msg_unsigned(
            pool.validator_index(), casper.current_epoch())
